...
const german: Record<string, string> = {
  ...
  "STOPPED": "GESTOPPT",
  "ERROR": "FEHLER",
  "Write your commands, then click Compile.": "Schreibe deine Befehle und klicke auf Ausführen.",
  ...
  "Return True if the selected tile has a fully grown plant; otherwise False.": "Gibt True zurück, wenn die Pflanze auf dem ausgewählten Feld ausgewachsen ist, sonst False.",
  "Sell a whole number of harvested wheat, tomato, or cucumber plants and add their value to your balance.": "Verkaufe eine ganze Anzahl geernteter Pflanzen (wheat, tomato oder cucumber). Der Erlös wird deinem Guthaben gutgeschrieben.",
  ...
  "Return the current (x, y) coordinates.": "Gibt die aktuellen Koordinaten (x, y) zurück.",
  ...
  "Return the harvested inventory count for wheat, tomato, or cucumber.": "Gibt die Anzahl geernteter Pflanzen für wheat, tomato oder cucumber zurück.",
  "Print a quoted string or value, such as print(get_position()). Single quotes work too.": "Gibt einen Text in Anführungszeichen oder einen Wert aus, etwa print(get_position()). Einfache Anführungszeichen funktionieren auch.",
  "Run the indented block once. if(False): skips it. Numeric comparisons are supported, such as get_x_cord() == 2.": "Führt den eingerückten Block einmal aus. if(False): überspringt ihn. Zahlenvergleiche wie get_x_cord() == 2 sind möglich.",
  ...
  "Run this indented block when the matching if is false. Align else with its if.": "Führt diesen eingerückten Block aus, wenn die zugehörige if-Bedingung falsch ist. else und if müssen gleich eingerückt sein.",
  "Both conditions must be true. Evaluated before or.": "Beide Bedingungen müssen wahr sein. Wird vor or ausgewertet.",
  ...
  "One statement per line. Tab inserts four spaces; Shift+Tab removes indentation. Press Escape then Tab to leave the editor. Commands and while/if checks each pause for 500 ms.": "Eine Anweisung pro Zeile. Tab fügt vier Leerzeichen ein; Umschalt+Tab entfernt die Einrückung. Mit Escape und anschließend Tab verlässt du den Editor. Befehle und while/if-Prüfungen pausieren jeweils 500 ms.",
  ...
  "left": "links",
  "right": "rechts",
  "Return to the upper-left tile, (0, 0), facing right.": "Kehre zum Feld oben links (0, 0) mit Blick nach rechts zurück.",
  "Remove every plant and return to (0, 0), facing right. Inventory and balance stay unchanged.": "Entferne alle Pflanzen und kehre zu (0, 0) mit Blick nach rechts zurück. Vorräte und Guthaben bleiben unverändert."
};
...
export function translatePage(language: Language): void {
  ...
  for (const attribute of ["placeholder", "aria-label"]) {
    document.querySelectorAll<HTMLElement>(`[data-i18n-${attribute}]`).forEach((element) => {
      element.setAttribute(attribute, translate(element.getAttribute(`data-i18n-${attribute}`)!, language));
    });
  }
}
