...
import { expandField, getExpansionCost, MAX_FIELD_SIZE } from "./farm.ts";
import { translate, translatePage, type Language } from "./i18n.ts";
...
const upgradeButton = document.querySelector<HTMLButtonElement>("#upgrade-field-button")!;
let farm = createFarm();
let language: Language = "en";
const languageSelector = document.querySelector<HTMLSelectElement>("#language-selector")!;
...
function setStatus(value: string): void { executionStatus = value; status.textContent = t(value); }
...
function render(): void {
  ...
  upgradeButton.textContent = expansionCost === undefined
    ...
    : `${t("Expand to")} ${farm.size + 1} × ${farm.size + 1} — ${expansionCost.toLocaleString(language === "de" ? "de-DE" : "en-US", { minimumFractionDigits: 2 })}`;
  const selected = farm.tiles[farm.position.y][farm.position.x].plant;
  field.setAttribute("aria-label", `${t("Farm field. Size:")} ${farm.size} × ${farm.size}. ${t("Selected tile:")} ${coordinates}. ${t("Facing")}: ${t(farm.direction)}. ${selected ? t(selected.name) + ", " + t(selected.isGrown(now) ? "ready to harvest" : "growing") + "." : t("Empty.")}`);
}
...
compileButton.addEventListener("click", async () => {
  ...
  try {
    const result = await runProgram(program.instructions, farm, ({ instruction, position, message, output: printed }) => {
      render();
      if (printed !== undefined) { writeLine(printed); return; }
