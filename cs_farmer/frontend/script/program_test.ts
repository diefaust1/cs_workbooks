...
Deno.test("movement grammar and source lines distinguish turning from moving", () => {
  deepStrictEqual(compileProgram('# start\nmove()\n\ndirection( down )\nmove()'), { ok: true, instructions: [
    { command: "move", line: 2 }, { command: "direction", argument: "down", line: 4 }, { command: "move", line: 5 },
  ] });
  for (const code of ['move(right)', 'direction()', 'direction(diagonal)', 'direction(right, left)', 'right()']) strictEqual(compileProgram(code).ok, false);
});
Deno.test("whole program validation rejects invalid lines and malformed indentation", () => {
  const result = compileProgram('move()\nwrong()\nmove(right)');
  deepStrictEqual(result, { ok: false, errors: [{ line: 2, source: 'wrong()' }, { line: 3, source: 'move(right)' }] });
  for (const code of ['while(True):', 'while(true):\n    move()', 'while(True):\nmove()', 'while(True):\n  move()', 'while(True):\n\tmove()', '    move()']) strictEqual(compileProgram(code).ok, false);
...
Deno.test("turns do not move, facing persists between runs, and resets restore right", async () => {
  ...
  const run = async (code: string) => {
    ...
    await runProgram(result.instructions, farm, () => {}, () => Promise.resolve());
  };
  await run('move()\ndirection(down)');
...
Deno.test("turning and movement each wait one fixed interval", async () => {
  ...
  const running = runProgram(result.instructions, farm, step => steps.push(step), ms => {
    waits.push(ms); return new Promise<void>(resolve => { release = resolve; });
  });
  strictEqual(farm.direction, 'right');
...
Deno.test("Stop cancels a pending turn before it changes facing", async () => {
  ...
  if (!result.ok) throw new Error('Invalid test program');
  const farm = createFarm(); const controller = new AbortController();
  let release!: () => void;
  const running = runProgram(result.instructions, farm, () => { throw new Error('Unexpected turn'); }, () => new Promise<void>(resolve => { release = resolve; }), controller.signal);
  controller.abort(); release();
  strictEqual(await running, 'stopped'); strictEqual(farm.direction, 'right');
});
Deno.test("true loops with only false loops keep yielding and can be stopped", async () => {
  const result = compileProgram('while(True):\n    while(False):\n        move()');
  if (!result.ok) throw new Error('Invalid test program');
  const farm = createFarm(); const controller = new AbortController(); let ticks = 0;
  strictEqual(await runProgram(result.instructions, farm, () => { throw new Error('Unexpected move'); }, () => {
    if (++ticks === 6) controller.abort(); return Promise.resolve();
