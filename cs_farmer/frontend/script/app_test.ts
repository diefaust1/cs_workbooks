...
Deno.test("Compile preserves position between clicks and resets only on reset()", async () => {
  ...
  element("#code-editor").setAttribute(
    "data-i18n-placeholder",
    "Insert code here...",
  );
  element("#language-selector").setAttribute(
  ...
  if (!bundleEntry) {
    throw new Error(
      "Vite JavaScript bundle not found. Run the production build first.",
    );
  }
  const bundle = await Deno.readTextFile(
    new URL(bundleEntry.name, assetsDirectory),
  );
  let ticks = 0;
  ...
  runInNewContext(bundle, {
    ...
    document: {
      ...
      querySelectorAll: (selector: string) =>
        [...elements.values()].filter((item) =>
          item.attributes.has(selector.slice(1, -1))
        ),
    },
  ...
  const compile = async (source: string) => {
    ...
    strictEqual(
      element("#field").children.filter((tile) =>
        tile.classes.has("is-selected")
      ).length,
      1,
    );
  };
  ...
  strictEqual(element("#seed-watermelon").textContent, "0");
  strictEqual(element("#harvest-watermelon").textContent, "0");
  ...
  match(
    ...
    /Expected a loop body indented by four spaces/,
  );
  ...
  strictEqual(
    element("#field").children.filter((tile) => tile.classes.has("has-plant"))
      .length,
    2,
  );
  ...
  strictEqual(
    element("#field").children.some((tile) => tile.classes.has("has-plant")),
    false,
  );
  strictEqual(element("#position").textContent, "Position: (0, 0)");
  ...
  strictEqual(
    element("#language-selector").getAttribute("aria-label"),
    "Sprache",
  );
  strictEqual(element("#balance").textContent, "0,50");
  ...
  await compile("reset()\ndirection(left)");
  let selected = element("#field").children.find((tile) =>
    tile.classes.has("is-selected")
  )!;
  ...
  selected = element("#field").children.find((tile) =>
    tile.classes.has("is-selected")
  )!;
  ...
  for (const reset of ["reset()", "reset_field()"]) {
    ...
    const active = element("#field").children.find((tile) =>
      tile.classes.has("is-selected")
    )!;
  ...
  element("#load-input").files = [{
    text: () =>
      Promise.resolve(JSON.stringify({
        ...
        harvest: { wheat: 0, tomato: 0, cucumber: 0 },
        plants: [],
        editorCode: "print(get_direction())",
      })),
  }];
  await element("#load-input").listener!();
  await compile("buy(tomato, 2)");
  strictEqual(element("#seed-tomato").textContent, "2");
  strictEqual(element("#balance").textContent, "4.00");
  strictEqual(element("#field").children.length, 36);
  strictEqual(
    element("#field").children.filter((tile) => tile.classes.has("is-locked"))
      .length,
    35,
  );
