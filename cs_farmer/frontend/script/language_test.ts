...
Deno.test("if true runs once, if false skips, and nested blocks dedent correctly", async () => {
  ...
  const outcome = await runProgram(
    result.instructions,
    farm,
    (step) => lines.push(step.instruction.line),
    () => {
...
import { evaluateCondition, parseCondition } from "./expressions.ts";
...
Deno.test("strings support equality comparisons and read direction at execution time", async () => {
  const result = compileProgram(
  ...
  deepStrictEqual(output, ["start", "turned", "equal", "symbols"]);
});
  ...
  for (const a of [false, true]) {
    for (const b of [false, true]) {
      for (const operator of ["and", "&", "or", "|"]) {
        ...
        strictEqual(
          evaluateCondition(condition, farm),
          operator === "and" || operator === "&" ? a && b : a || b,
        );
      }
    }
  }
...
Deno.test("logical evaluation short circuits but parsing validates skipped operands", () => {
  const farm = createFarm();
  Object.defineProperty(farm.position, "x", {
    get: () => {
      throw new Error("Should not evaluate x");
    },
  });
  for (
    const source of ["False and get_x_cord() == 0", "True or get_x_cord() == 0"]
  ) {
    const condition = parseCondition(source);
    if (condition === undefined) throw new Error(source);
...
Deno.test("nested else binds at its indentation and executes only the chosen branch", async () => {
  ...
  await runProgram(result.instructions, createFarm(), (step) => {
    ...
    if (++ticks > 20) throw new Error("Branch repeated");
    return Promise.resolve();
  });
...
Deno.test("compound while and if else use current state each iteration", async () => {
  const result = compileProgram(
    "while(get_x_cord() < 2 | get_y_cord() < 2):\n    if(get_x_cord() < 2 & True):\n        move()\n    else:\n        direction(down)\n        move()\nprint(get_position())",
  );
  if (!result.ok) throw new Error(JSON.stringify(result.errors));
  const farm = createFarm(5);
  let ticks = 0;
